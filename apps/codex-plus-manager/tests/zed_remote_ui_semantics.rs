...
use codex_plus_manager::state::zed_remote::ZedRemoteViewState;
use codex_plus_manager::theme;
use codex_plus_manager::views::shell::{ShellFeatureStates, render_shell};
use codex_plus_manager::views::zed_remote::{self, ZedRemoteAction};
...
fn settings_conflict_and_partial_success_keep_actionable_operational_state() {
    ...
    assert!(conflict.apply_save_response(
        save_id,
        Err(
            codex_plus_manager::state::zed_remote::ZedRemoteFailureKind::Service(
