...
fn snapshot(codex_version: &str) -> Arc<OverviewSnapshot> {
...
fn model(locale: Locale, theme: ThemeMode) -> ShellViewModel {
    ShellViewModel {
        ...
        overview_error: None,
        last_updated: Some("12:34:56".to_owned()),
...
fn render_test_shell(ui: &mut egui::Ui, state: &mut TestShellState) {
    ...
    for action in render_shell(ui, &state.model) {
